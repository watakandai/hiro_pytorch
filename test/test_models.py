...
import torch
from hiro.models import HigherActor, get_tensor

class ModelsTest(unittest.TestCase):
    def test_highactor_output_size(self):
        max_action = get_tensor(np.random.randint(5, size=5))
        actor = HigherActor(10, 3, 5, max_action)
    ...
    def test_highactor_output_type(self):
        max_action = get_tensor(np.random.randint(5, size=5))
        actor = HigherActor(10, 3, 5, max_action)
        state = get_tensor(np.random.rand(10))
        goal = get_tensor(np.random.rand(3))
    ...
    def test_highactor_output_minmax(self):
        random_value = 100
        sdim = 10
        gdim = 3
        adim = 5
        max_action = get_tensor(np.random.randint(random_value, size=adim))
        actor = HigherActor(sdim, gdim, adim, max_action)
